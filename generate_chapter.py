...
def generate_chapter(chapter, script_dir, silent=False, output_file=None):
    ...
    target_words = get_word_count_target() // get_default_chapters()  # Divide total by chapter count

    prompt = f"""{system_prompt.format(WORD_COUNT_TARGET=target_words)}
...
Aim for 50% dialog, 25% narration, 15% body language and 10% thoughts.
